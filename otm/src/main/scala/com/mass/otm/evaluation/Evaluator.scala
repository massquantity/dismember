...
object Evaluator extends Serializable with CandidateSearcher {
  ...
  implicit val criterions =
    (1 to threadNum).map(_ => BCECriterionWithLogits[Double](sizeAverage = false))
  ...
  def evaluate(
    ...
    val (totalLoss, totalMetrics) = dataset.evalData
      ...
      .flatMap { batchData =>
        ...
        batchData
          ...
          .toSeq
          .zipWithIndex
          .map { case (threadData, i) =>
            Future {
              val batchNodes =
                batchBeamSearch(threadData, models(i), tree, beamSize, seqLen, useMask)
              val (batchPreds, batchLabels, batchMetrics) = threadData
                .zip(batchNodes)
                .map { case (data, nodes) =>
                  ...
                  val labels = leafNodes.map { n =>
                    data.targetItems.find(_ == n.id) match {
                      case Some(_) => 1.0
                      case None => 0.0
                    }
                  }
