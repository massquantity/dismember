...
import com.mass.sparkdl.tensor.{DenseTensorApply => Apply}
...
object DenseTensorMath {
  ...
  def addmv[@specialized(Float, Double) T](r: Tensor[T], beta: T, t: Tensor[T], alpha: T,
    ...
    if (mat.stride(0) == 1) {
      ...
      ev.gemv('N', mat.size(0), mat.size(1), alpha, mat.storage().array(), mat.storageOffset(),
        lda, vec.storage().array(), vec.storageOffset(), vec.stride(0), beta,
        r.storage().array(), r.storageOffset(), r.stride(0))
    } else if (mat.stride(1) == 1) {
      ev.gemv('T', mat.size(1), mat.size(0), alpha, mat.storage().array(), mat.storageOffset(),
        mat.stride(0), vec.storage().array(), vec.storageOffset(), vec.stride(0), beta,
        r.storage().array(), r.storageOffset(), r.stride(0))
    } else {
      val cmat = mat.contiguous()
      ev.gemv('T', cmat.size(1), cmat.size(0), alpha, cmat.storage().array(),
        cmat.storageOffset(), cmat.stride(0), vec.storage().array(), vec.storageOffset(),
        vec.stride(0), beta, r.storage().array(), r.storageOffset(), r.stride(0))
    }
    r
  }

  def addr[@specialized(Float, Double) T](r: Tensor[T], beta: T, t: Tensor[T], alpha: T,
      vec1: Tensor[T], vec2: Tensor[T])(implicit ev: TensorNumeric[T]): Tensor[T] = {
    ...
    if (r.stride(0) == 1) {
      ...
      ev.ger(vec1.size(0), vec2.size(0), alpha, vec1.storage().array(), vec1.storageOffset(),
        vec1.stride(0), vec2.storage().array(), vec2.storageOffset(), vec2.stride(0),
        r.storage().array(), r.storageOffset(), lda)
    } else if (r.stride(1) == 1) {
      ev.ger(vec2.size(0), vec1.size(0), alpha, vec2.storage().array(), vec2.storageOffset(),
        vec2.stride(0), vec1.storage().array(), vec1.storageOffset(), vec1.stride(0),
        r.storage().array(), r.storageOffset(), r.stride(0))
    } else {
      val cr = r.contiguous()
      ev.ger(vec2.size(0), vec1.size(0), alpha, vec2.storage().array(), vec2.storageOffset(),
        vec2.stride(0), vec1.storage().array(), vec1.storageOffset(), vec1.stride(0),
        cr.storage().array(), cr.storageOffset(), cr.stride(0))
      r.copy(cr)
    }
    r
  }
  ...
  def cadd[@specialized(Float, Double) T](self: DenseTensor[T], x: Tensor[T], value: T,
      y: Tensor[T])(implicit ev: TensorNumeric[T]): Tensor[T] = {
    require(x != null && y.nElement() == x.nElement())

    if (!self.eq(x) && !self.eq(y)) {
      self.resizeAs(x).copy(x)
    }
    ev.axpy(y.nElement(), value, y.storage().array(), y.storageOffset(), 1,
  ...
  def cmul[@specialized T](self: DenseTensor[T], x: DenseTensor[T], y: DenseTensor[T])(
      implicit ev: TensorNumeric[T]): Tensor[T] = {
    ev.vMul(self.nElement(), x.storage().array(), x.storageOffset(), y.storage().array(),
      y.storageOffset(), self.storage().array(), self.storageOffset())
    self
  }

  def cdiv[@specialized(Float, Double) T](self: DenseTensor[T], x: Tensor[T], y: Tensor[T])(
      implicit ev: TensorNumeric[T]): Tensor[T] = {
    ev.vDiv(self.nElement(), x.storage().array(), x.storageOffset(), y.storage().array(),
      y.storageOffset(), self.storage().array(), self.storageOffset())
    self
  }
  ...
  def log[@specialized(Float, Double) T: ClassTag](self: DenseTensor[T], x: Tensor[T])(
      implicit ev: TensorNumeric[T]): Tensor[T] = {
    require(self.nElement() == x.nElement())
    ev.vLn(self.nElement(), x.storage().array(), x.storageOffset(),
      self.storage().array(), self.storageOffset())
    self
  }
  ...
  def pow[@specialized(Float, Double) T: ClassTag](self: DenseTensor[T], x: Tensor[T], n: T)(
      implicit ev: TensorNumeric[T]): Tensor[T] = {
    require(self.nElement() == x.nElement())
    ev.vPowx(self.nElement(), x.storage().array(), x.storageOffset(), n,
      self.storage().array(), self.storageOffset())
    self
  }
  ...
  def exp[@specialized(Float, Double) T: ClassTag](self: DenseTensor[T], x: Tensor[T])(
    ...
    ev.vExp(self.nElement(), x.storage().array(), x.storageOffset(),
      self.storage().array(), self.storageOffset())
    self
  }
  ...
  def sqrt[@specialized(Float, Double) T: ClassTag](self: DenseTensor[T], x: Tensor[T])(
      implicit ev: TensorNumeric[T]): Tensor[T] = {
    ev.vSqrt(self.nElement(), x.storage().array(), x.storageOffset(),
      self.storage().array(), self.storageOffset())
    self
  }
